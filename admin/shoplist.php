...
<nav class="navbar navbar-light justify-content-center fs-3 mb-5" style="background-color: #00ff5573;">
  ...
  <i class='bx bxs-user'></i>Admin | Shop List
  </nav>
    ...
    <table class="table table-hover text-center">
      ...
      <?php
        ...
        while($row = oci_fetch_array($stid,OCI_ASSOC)){
            ...
            echo "
            <tr>
                <th scope='col'>".$row['SHOP_ID']."</th>
                <th scope='col'>".$row['SHOP_NAME']."</th>
                <th scope='col'>".$row['TRADER_CATEGORY']."</th>
                <th scope='col'>".$row['SHOP_DESC']."</th>
                <th scope='col'>".$row['STATUS']."</th>
                <th scope='col' class='d-flex justify-content-center gap-2'>";
                    if($row['STATUS'] === 'pending'){
                        echo "<a href='updateshop.php?id=$id&status=verified' >Approve</a>";
                        echo "<a href='updateshop.php?id=$id&action=delete' >Decline</a>";
                    }
                    else{
                        echo "<a href='updateshop.php?id=$id&status=pending' >Deactivate</a>";
                    }   
                echo "</th>";
            echo "</tr>";
        }
